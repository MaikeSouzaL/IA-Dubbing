...
from project_paths import pipeline_file, video_original_file, work_file
...
if __name__ == "__main__":
    try:
        with open(video_original_file(), "r", encoding="utf-8") as f_video:
    ...
    transcricao = pipeline_file("transcricao.json")
    frases_pt = pipeline_file("frases_pt.json") if pipeline_file("frases_pt.json").is_file() else work_file("frases_pt.json")
    ...
    ret = subprocess.run([sys.executable, str(ROOT / "scripts" / "sincronizar_e_juntar.py")], encoding='utf-8', errors='replace')
    if ret.returncode != 0:
