...
# Carrega transcrição
try:
    data = load_json(pipeline_file("transcricao.json"))
...
frases_pt_path = work_file("frases_pt.json")
save_json(frases_pt, frases_pt_path)
copy_artifact(frases_pt_path, "frases_pt.json")
...
if not frases_pt:
    ...
    transcricao_pt_path = pipeline_file("transcricao_pt.json")
    if os.path.isfile(transcricao_pt_path):
        try:
            data_pt = load_json(transcricao_pt_path)
            fallback = []
            ...
            save_json(fallback, frases_pt_path)
...
ret = subprocess.run([sys.executable, str(ROOT / "scripts" / "dublar_frases_pt.py")], check=False, encoding='utf-8', errors='replace')
if ret.returncode != 0:
