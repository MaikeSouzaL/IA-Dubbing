...
def _add_nvidia_dll_directories():
    """Adiciona DLLs CUDA instaladas via pip ao processo no Windows."""
    if os.name != "nt":
        return
...
class ModelCache:
    ...
    def get_faster_whisper(self, model_size: str = "large-v3-turbo") -> Any:
        """Carrega ou retorna modelo Faster-Whisper do cache."""
        if WhisperModel is None:
            raise RuntimeError(
                "Biblioteca faster-whisper nao instalada. Rode: pip install -r requirements.txt"
            )

        device_config = config.get("models.faster_whisper.device", "auto")
        if device_config == "auto":
            device = "cuda" if torch.cuda.is_available() else "cpu"
        else:
            device = device_config

        compute_type = config.get("models.faster_whisper.compute_type", None)
        if not compute_type:
            compute_type = "int8_float16" if device == "cuda" else "int8"

        cache_key = f"faster_whisper_{model_size}_{device}_{compute_type}"
        if cache_key in self._memory_cache:
            logger.debug(f"Faster-Whisper ({model_size}) carregado do cache.")
            return self._memory_cache[cache_key]

        logger.info(
            f"Carregando Faster-Whisper ({model_size}) no {device.upper()} "
    ...
    def _configure_tts_home(self) -> Path:
        ...
        if cfg_home:
            ...
            if not home_path.is_absolute():
                home_path = (ROOT / home_path).resolve(strict=False)
        else:
            home_path = (ROOT / self.cache_dir / "tts_home").resolve(strict=False)
