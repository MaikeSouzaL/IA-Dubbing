...
from project_paths import outputs_dir, pipeline_file, temp_dir, video_original_file, work_file
...
def limpar():
    ...
    try:
        original_marker = video_original_file() if video_original_file().is_file() else pipeline_file("video_original.txt")
        if os.path.exists(original_marker):
            with open(original_marker, "r", encoding="utf-8") as f:
    ...
    files_to_remove = [
        ...
        "audio_final_mix.wav",
        "video_original.txt",
        work_file("transcricao.json"),
        work_file("transcricao_pt.json"),
        work_file("frases_pt.json"),
        work_file("voz_dublada.wav"),
        work_file("audio_final_mix.wav"),
        video_original_file(),
    ]
    ...
    dirs_to_remove = [
        ...
        config.get("paths.separated_dir", "data/work/separated"),
        str(temp_dir()),
        "separated",  # estrutura antiga
        "temp",
    ]
    ...
    videos_dublados = glob.glob("*_dublado.mp4") + glob.glob(str(outputs_dir() / "*_dublado.mp4"))
    if videos_dublados:
        logger.info("=" * 60)
